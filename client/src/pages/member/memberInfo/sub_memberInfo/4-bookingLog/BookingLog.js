...
const _EHS = new ExceptionHandleService({
  _NAME: 'page/member/memberInfo/sub_memberInfo/3-bookingLog/BookingLog.js',
  _NOTICE: ''
});
...
function BookingLog() {
  ...
  const columns = [
    ...
    columnHelper.accessor('isCheckIn', {
      header: '是否報到',
      size: 160,
      cell: (info) => (
        <div>
          {info.getValue() ? <p>是</p> : <p className='text-danger'>否</p>}
        </div>
      )
    }),
  ...
  const getList = useCallback(async (reqBody) => {
    try {
      const res = await api.member.getMemberBookingLog(reqBody);
      if (res) {
        ...
        const formatOrderItem = (item) => {
          return {
            ...
            peopleCount: item?.peopleCount
              ? insertComma(Number(item?.peopleCount))
              : 0,
            bookingId: item?.bookingId || '--',
            bookingDate: item?.bookingDate || '--',
            isCheckIn: item?.isCheckIn || 0
          };
        };
  ...
  const handleSelect = (name, selectedValue, isCustomDateRange = false) => {
    ...
    if (name === 'date') {
      switch (selectedValue) {
        case '90':
          methods.setValue('startDate', rangeDate(-90));
          methods.setValue('endDate', new Date());
          setShowCustomDatePopover(false);
          break;
        case '180':
          methods.setValue('startDate', rangeDate(-180));
          methods.setValue('endDate', new Date());
          setShowCustomDatePopover(false);
          break;
        case '365':
          methods.setValue('startDate', rangeDate(-365));
          methods.setValue('endDate', new Date());
          setShowCustomDatePopover(false);
          break;
        case 'custom':
